...
/// @notice Pure math for volatility calculations.
///         Computes price returns and EWMA volatility updates.
///         Does NOT store anything — no mappings, no state variables.
///         Receives inputs → computes → returns outputs.
library Volatility {
    // ──────────────────────── Constants ────────────────────────

    /// @dev Fixed-point scale (1e18) used for all intermediate math.
    uint256 internal constant SCALE = 1e18;

    /// @dev EWMA smoothing factor α (alpha).
    ///      0.1e18 = 10%  →  newEWMA = 0.1 * currentReturn + 0.9 * oldEWMA
    ///      Tune this to control how fast the EWMA reacts to new data.
    uint256 internal constant ALPHA = 0.1e18;

    /// @dev Complement of alpha: (1 - α) pre-computed for gas savings.
    ...
    /// @notice Calculates the absolute percentage return between two sqrtPriceX96 values.
    /// @dev    return = | newPrice - oldPrice | / oldPrice   (scaled by 1e18)
    ...
    /// @param oldSqrtPriceX96 The previous sqrtPriceX96.
    /// @param newSqrtPriceX96 The current  sqrtPriceX96.
    /// @return absReturn The absolute percentage return, scaled by 1e18.
    function calculateReturn(
        ...
        // Scale to 1e18 fixed-point percentage
        absReturn = (diff * SCALE) / oldPrice;
    }

    // ──────────────────────── B) EWMA Update ────────────────────────

    /// @notice Updates the EWMA volatility estimate with a new return observation.
    /// @dev    newEWMA = α * currentReturn + (1 - α) * oldEWMA
    /// @param oldEwma       The previous EWMA volatility (scaled 1e18).
    /// @param currentReturn The latest absolute return   (scaled 1e18).
    /// @return newEwma      The updated EWMA volatility  (scaled 1e18).
    function updateEwma(
        uint256 oldEwma,
    ...
    /// @notice Full volatility computation pipeline.
    ...
    /// @param oldState        The previous VolatilityState from storage.
    /// @param newSqrtPriceX96 The current sqrtPriceX96 after the swap.
    /// @return updatedState   The new VolatilityState to be written back by the caller.
    function compute(
